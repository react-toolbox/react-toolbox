...
class Hours extends Component {
  ...
  state = {
    inner: this.props.format === '24hr' &&
      this.props.selected > 0 &&
      this.props.selected <= 12,
  };

  handleHandMove = (degrees, radius) => {
    const currentInner = radius <
      this.props.radius - this.props.spacing * innerSpacing;
  ...
  valueFromDegrees(degrees) {
    if (
      this.props.format === 'ampm' ||
      (this.props.format === '24hr' && this.state.inner)
    ) {
    ...
    return (
      <div>
        ...
        <Hand
          ...
          angle={selected * step}
          length={
            (this.state.inner ? radius - spacing * innerSpacing : radius) -
              spacing
          }
