...
describe('Slider', () => {
  describe('#positionToValue', () => {
    it('returns min when position is less than origin', () => {
      const instance = shallow(
        <Slider theme={theme} min={-500} max={500} />
      ).instance();
  ...
  describe('#trimValue', () => {
    it('rounds to the proper number', () => {
      const instance = shallow(
        <Slider theme={theme} min={0} max={100} step={0.1} />
      ).instance();
  ...
  describe('#valueForInput', () => {
    it('returns a fixed number when an integer is given', () => {
      const instance = shallow(
        <Slider theme={theme} min={0} max={100} step={0.01} />
      ).instance();
  ...
  describe('#knobOffset', () => {
    it('returns the corresponding offset for a given value and slider length/start', () => {
      const slider = shallow(
        <Slider theme={theme} min={-500} max={500} value={-250} />
      ).instance();
  ...
  describe('#render', () => {
    it('contains a linear progress bar with proper properties', () => {
      const wrapper = mount(
        <Slider theme={theme} min={100} max={1000} value={140} />
      );
  ...
  describe('#events', () => {
    ...
    it('changes input value when slider changes', () => {
      ...
      const wrapper = mount(
        <Slider theme={theme} editable onChange={onChange} />
      );
      const instance = wrapper.instance();
      instance.setState({ sliderStart: 0, sliderLength: 1000 });
      instance.handleResize = (evt, callback) => {
