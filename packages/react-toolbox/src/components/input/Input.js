  ...
  class Input extends React.Component {
    ...
    handleKeyPress = event => {
      ...
      if (multiline && maxLength) {
        ...
        // increase.
        const isReplacing = event.target.selectionEnd -
    ...
    render() {
      ...
      const valuePresent = this.valuePresent(value) ||
        this.valuePresent(defaultValue);
      ...
      return (
        <div data-react-toolbox="input" className={className}>
          ...
          {labelText
            ...
            : null}
          {hint
            ? <span hidden={labelText} className={theme.hint}>{hint}</span>
            : null}
          {error ? <span className={theme.error}>{error}</span> : null}
          {maxLength
            ? <span className={theme.counter}>{length}/{maxLength}</span>
            : null}
          {children}
        </div>
      );
