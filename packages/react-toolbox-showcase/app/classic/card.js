...
import { Button, IconButton } from 'react-toolbox/src/components/button';
import {
  Card,
  CardActions,
  CardMedia,
  CardText,
  CardTitle,
} from 'react-toolbox/src/components/card';
...
const CardList = ({ children }) => (
  <ul className={style.cardsGroup}>{children}</ul>
);
...
const CardTest = () => (
  <section>
    <h5>Cards</h5>
    <p>
      You have multiple options for cards. Combine different subcomponents to create your own:
    </p>

    <div className={style.cards}>
      {Object.keys(cards).map(key => (
        <CardList key={key}>
          {cards[key].map((demo, i) => (
            <CardListItem key={key + i} {...demo} />
          ))}
        </CardList>
      ))}
    </div>
