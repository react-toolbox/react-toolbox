...
function getConfig(config) {
  const filterInput = R.pickBy(
    (v, k) => !R.isNil(v) && R.contains(k, C.INPUT_OPTIONS)
  );
...
function getBuildPaths(options) {
  const cssName = getFileName(
    options[C.CSS_OPTION],
    C.DEFAULT_FILENAME,
    '.css'
  );
  const jsName = getFileName(options[C.JS_OPTION], C.DEFAULT_FILENAME, '.js');
  const cssDir = getFileDir(
    options[C.CSS_OPTION],
    options[C.OUTPUT_OPTION] || C.DEFAULT_OUTPUT
  );
  const jsDir = getFileDir(
    options[C.JS_OPTION],
    options[C.OUTPUT_OPTION] || C.DEFAULT_OUTPUT
  );
...
function ifEmptyReturn(optionValue, defaultValue) {
  return optionValue === '' ? defaultValue : optionValue;
}
