...
class Face extends Component {
  ...
  numberStyle(rad, num) {
    ...
    return {
      position: 'absolute',
      left: (rad + rad * Math.sin(360 * (Math.PI / 180) / count * (num - 1)) + this.props.spacing),
      top: (rad - rad * Math.cos(360 * (Math.PI / 180) / count * (num - 1)) + this.props.spacing),
    };
  }
  ...
  renderNumber = (number, idx) => {
    const { theme, active, radius, spacing, twoDigits } = this.props;
    return (
      <span
