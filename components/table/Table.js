  ...
  class Table extends Component {
    ...
    handleHeadSelect = value => {
      if (this.props.onRowSelect) {
        this.props.onRowSelect(value ? this.getRowTuples().map(item => item[0]) : []);
      }
    };
    ...
    renderHead = () => {
      ...
      return React.Children.map(filterReactChildren(this.props.children, isTableHead), child =>
        cloneElement(child, {
          selected,
    ...
    renderRows = () =>
      React.Children.map(filterReactChildren(this.props.children, isTableRow), (child, idx) =>
        cloneElement(child, {
          idx,
